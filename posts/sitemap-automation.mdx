---
title: '수백만 동적 페이지를 위한 오늘의집 커뮤니티 Sitemap 자동화 시스템 구축기'
publishedAt: '2025-09-13'
summary: '오늘의집 커뮤니티의 SEO 개선을 위해 수백만 개의 동적 페이지를 자동으로 관리하는 사이트맵 시스템을 구축한 경험을 공유합니다.'
---

안녕하세요! 오늘의집 콘텐츠 > 커뮤니티 팀의 Software Engineer 모리입니다. 🐤

오늘의집에 합류하여 처음 맡은 과업은 커뮤니티의 SEO(검색 엔진 최적화)를 개선하는 프로젝트였습니다. 저희 팀의 핵심 목표(KR)는 **웹 페이지의 외부 유입을 늘려 일간 커뮤니티 활성 사용자(AU)를 성장시키는 것**입니다. 사용자가 구매 결정을 내리는 과정에서 겪는 문제를 해결해 주는 커뮤니티를 만드는 것이 저희의 미션이며, 이 목표를 달성하는 데 SEO는 가장 중요한 열쇠 중 하나였습니다.
...
일반적인 정적 웹사이트라면 사이트맵 생성기를 이용해 `sitemap.xml` 파일을 만들어 제출하면 됩니다. 하지만 오늘의집 커뮤니티는 상황이 달랐습니다.

- **커뮤니티 게시글:** 사용자가 글을 작성할 때마다 상세 페이지 URL이 동적으로 생성됩니다.
- **카테고리:** 어드민에서 카테고리를 추가하거나 수정하면 관련 페이지 URL이 변경됩니다.
...
콘텐츠팀의 코드를 살펴보던 중 sitemap과 관련된 코드를 발견했습니다. 이미 `next-sitemap` 이란 라이브러리로 글로벌팀에서 시도한 흔적이 있었습니다. 처음에는 `next-sitemap` 라이브러리를 사용해 손쉽게 해결하고자 했습니다. 기존에 사용하던 커뮤니티 목록 조회 API를 호출하여 게시글 ID를 가져와 동적으로 사이트맵을 생성하는 방식이었습니다.
...
1. **데이터베이스 부하:** 기존 API는 페이징(Paging) 처리되어 있어 150만 개가 넘는 게시글 ID를 모두 가져오려면 수많은 API를 연속 호출해야 했습니다. 전체 ID를 한 번에 응답하는 API를 새로 개발하더라도, 데이터 규모가 커질수록 DB에 심각한 부하를 줄 것이 뻔했습니다.
...
먼저 사이트맵 파일을 저장할 S3 버킷 구조를 설계했습니다. 확장성을 고려하여 `/content/sitemap/community/` 경로 아래에 커뮤니티 관련 사이트맵을 모두 모으고, 역할별로 폴더를 분리했습니다.
...
- **커뮤니티 상세 페이지:** `https://contents.ohou.se/community/posts/{slug}`
...
- **카테고리 페이지:**
    - 메인: `https://contents.ohou.se/community/{mainCategory}`
    - 서브: `https://contents.ohou.se/community/{mainCategory}?subCategory={subCategory}`
...
jobs:
  generate-sitemap:
    runs-on:
      group: prod
      labels: default-scale-set
    steps:
      # ... (Node.js, 의존성 설치 등)
      - name: Generate sitemap
        run: pnpm run tool:sitemap
        env:
          AWS_S3_ACCESS_KEY_ID_PROD: ${{ secrets.AWS_S3_ACCESS_KEY_ID_PROD }}
          AWS_S3_SECRET_ACCESS_KEY_PROD: ${{ secrets.AWS_S3_SECRET_ACCESS_KEY_PROD }}
```
...
function generateCommunitySitemapIndex(files) {
  const sitemapEntries = files.map(file => `
    <sitemap>
      <loc>https://contents.ohou.se/${file.name}</loc>
...
async function uploadSitemapIndex(xmlContent) {
  const key = `${S3_PATH_PREFIX}community.sitemap.xml`;
...
마지막으로, 사용자가 `https://contents.ohou.se/community.sitemap.xml`과 같은 서비스 URL로 접근했을 때, 실제로는 S3에 저장된 파일을 제공하도록 Next.js의 `rewrites` 기능을 활용했습니다.
...
const rewrites = [
  // 1. 인덱스 사이트맵 연결
  {
    ...
    destination: `${process.env.S3_HOST}/apne2/content/sitemap/community/community.sitemap.xml`,
  },
  // 2. 개별 상세 페이지 사이트맵 연결
  {
    source: '/:filename.community.sitemap.xml',
    destination: `${process.env.S3_HOST}/apne2/content/sitemap/community/post/:filename.xml`,
  },
  // 3. 카테고리 페이지 사이트맵 연결
  {
    source: '/category.community.sitemap.xml',
    destination: `${process.env.S3_HOST}/apne2/content/sitemap/community/category/category.community.sitemap.xml`,
  },
];
```
...
📊 Statistics:
  ...
  Post files: 12
  Category files: 3
  Upload URL: https://ohou-prs.s3.ap-northeast-2.amazonaws.com/...
```
...
- **확장성:** 커뮤니티뿐 아니라, 다른 콘텐츠, 토픽 등 다른 도메인 영역에서도 충분히 구조만 맞추어 사용 가능합니다. 또한 20만 개가 넘는 페이지는 물론, 앞으로 수백만 개로 늘어나더라도 감당할 수 있는 구조입니다.
...
또한, 이번 사이트맵 자동화는 단독적인 개선 작업으로 그치지 않습니다. 현재 SEO 개선 과제로 함께 진행하고 있는 **리치 스니펫 적용, 시맨틱 마크업 최적화**와 같은 다른 SEO 개선 과제들과 맞물려 **시너지 효과**를 낼 것으로 예상됩니다. 잘 구축된 사이트맵이 노출수를 증가시키는 역할을 하고, 시맨틱 마크업과 리치 스니펫 등 작업이 클릭율을 증가시키는 역할을 함으로써, 종합적인 관점에서 전체 SEO 유입량을 극대화하는 것이 최종 목표입니다.
...
아직 프로덕션 환경에 배포되기 전이라 실제 운영 중 모니터링을 해보지 못한 점은 아쉽지만, 곧 실제 환경에서의 성과를 확인할 수 있을 것으로 기대됩니다.
