...
팀 주제는 레거시인 Nexacro를 현대화하는 것이었습니다. Nexacro가 뭔지 모르는 저는 조사부터 시작했습니다. 팀은 6명이 2명씩 3조로 나뉘어 각자의 방식으로 PoC를 진행하기로 했고, 저는 Nexacro가 XML로 되어있다는 점에 착안하여 컴포넌트를 분석해서 React로 변환하는 주제를 택했습니다.

다만 Nexacro가 뭔지도 모르는 상태에서 워크샵을 선뜻 진행하기가 두려웠습니다. 그래서 먼저 PoC의 PoC를 진행했습니다. Nexacro의 화면 정의 파일인 `.xfdl`을 처음 열었을 때, 수천 줄의 XML 태그 안에 컴포넌트 정의, 데이터셋, 이벤트 핸들러가 선언적으로 들어있었습니다. "이거 React 컴포넌트랑 구조가 똑같잖아?"라는 생각이 들었고, Claude Code에 xfdl을 던져봤더니 완벽하지는 않았지만 구조를 읽고 React 컴포넌트로 변환하는 것이 가능했습니다. 다행히 결과가 잘 나와주었고, 방향성에 확신을 갖고 고도화하기로 했습니다.
...
마이그레이션 대상 시스템은 Nexacro 기반 **1,700개+ 화면**으로 구성된 사내 백오피스였습니다. 화면당 500~6,000줄, 수동 전환 시 추정 공수는 **약 4,300 M/D** (대략 20명이 1년). 착안점은 이것이었습니다:
...
처음에는 ChatGPT에 "이 xfdl을 React로 변환해줘"라고 해봤습니다. Nexacro 변환 규칙을 매번 설명해야 하고, 같은 프롬프트로도 매번 다른 결과가 나오고, 1,700개를 하나씩 프롬프트로 변환하는 건 불가능했습니다.

멀티 에이전트 시스템은 근본적으로 다릅니다. 프롬프트는 1회성 지시이고, 에이전트는 팀 빌딩입니다. 한 번 만든 팀이 1,700개 화면을 반복해서 처리합니다. 그리고 스펙(AC)이 있어야 자동 재시도가 가능합니다. "뭐가 맞는지" 기준이 없으면 AI가 실패해도 고칠 수 없습니다.
...
AI 코드 생성 품질이 가장 높은 프레임워크입니다. 학습 데이터 양이 압도적이고, 레거시 탈출 후 유지보수까지 고려하면 가장 합리적인 선택이었습니다.

### Zustand

Nexacro의 Dataset은 2차원 테이블 구조입니다. Redux는 보일러플레이트가 과다하고, Jotai/Recoil은 atom 단위라 Dataset 매핑이 어려웠습니다. Zustand는 슬라이스 패턴으로 Dataset → Store 1:1 매핑이 자연스럽고 보일러플레이트가 최소여서 선택했습니다.
...
- **shadcn/ui + Tailwind CSS**: 소스코드를 직접 복사하는 방식이라 커스텀이 자유롭고, MCP로 설치를 자동화할 수 있습니다. 에이전트가 `npx shadcn add`를 직접 실행합니다. `bg-primary`, `text-muted-foreground` 같은 시맨틱 컬러 토큰을 쓰면 202개 화면의 스타일이 자연스럽게 통일됩니다.
...
202개 화면 빌드가 2.58초입니다. 배치마다 빌드 검증을 해야 하므로 빌드 속도가 곧 생산성입니다.

### Axios + fast-xml-parser (nexacro-adapter)

Nexacro 백엔드가 XML 응답을 반환합니다. `nexacro-adapter` 패턴으로 기존 백엔드를 한 줄도 안 바꾸고 React에서 호출할 수 있게 만들었습니다.
...
AWS AI-DLC 워크샵에서 kiro-cli 토큰을 받아 처음 사용했습니다. 평소 Claude Code만 쓰다가 Kiro를 접한 비교입니다.
...
Kiro의 `.kiro/` 디렉토리 구조(steering, agents, skills, hooks)를 차용해서 에이전트를 JSON으로 정의하되, 실행은 Claude Code의 자율 모드로 돌렸습니다. 1,700개 화면을 매번 승인하며 변환할 수는 없었기 때문입니다. Kiro의 구조화된 설정 방식 덕분에 에이전트 관리가 훨씬 체계적이 됐습니다.
...
2. **스티어링의 힘**: 변환 규칙 211줄이 1,700개 화면의 품질을 결정합니다. 부족할 때마다 규칙을 추가하니 품질이 계단식으로 올라갔습니다.
3. **학습 루프**: 실수를 누적하고 다음 배치에 자동 반영하는 구조. Batch 1에서 6건의 빌드 에러가 났지만, Batch 2에서는 0건이었습니다.
4. **품질 게이트**: "80% 통과" 같은 명확한 기준이 있어야 자동화가 돌아갑니다.
5. **원본 데이터 충실도**: xfdl 파싱을 건너뛰면 빈 껍데기가 됩니다. 가장 비싼 교훈이었습니다.
6. **패턴 카탈로그**: 1,700개를 6가지 패턴으로 수렴시키면 파이프라인 하나로 전부 처리 가능합니다.
7. **점진적 자동화**: 처음부터 완벽한 파이프라인은 없습니다. PoC 8개로 시작해서 고도화한 것이 중요했습니다.

---

## 8. AI가 잘 못하는 것 — Super Hard의 벽

Super Hard 화면의 자동변환율은 40%입니다. 나머지 60%는 왜 AI가 못하는지 분석해봤습니다.
...
AI가 잘하는 영역은 "구조 변환"이고, 못하는 영역은 "비즈니스 로직 이해"입니다. Super Easy~Easy는 구조 변환만으로 90%+ 해결되고, Hard~Super Hard는 구조 변환 후 비즈니스 로직을 사람이 보정해야 합니다. 이 경계를 아는 것이 파이프라인 설계에서 가장 중요했습니다.
...
2. **에이전트를 4개로 시작했을 것입니다.** 실제로는 orchestrator → analyzer → codegen → test-runner 4개가 핵심이고, 나머지 4개는 고도화 과정에서 추가한 것입니다.
3. **패턴 카탈로그를 더 일찍 만들었을 것입니다.** 1,629개를 전수 스캔해서 6가지 패턴으로 분류한 건 Day 2였는데, Day 1에 했으면 첫날 실험의 정확도가 훨씬 높았을 겁니다.
4. **생성된 코드의 한계를 문서화했을 것입니다.** AI가 생성한 코드에는 `as any` 타입 캐스팅이 종종 있고, 일부 store 접근 패턴이 최적이 아닌 경우가 있습니다. 이런 특성을 미리 정리해두면 코드 리뷰 시간을 줄일 수 있습니다.
...
여태 바이브코딩이나 프롬프트를 주고받으며 Claude Code를 사용하는 것에 그쳤는데, 이번 워크샵에서 본격적으로 Kiro CLI를 다뤄보며 커스텀 에이전트를 구축하고, 필요한 Skill을 만들고, Steering에 지식을 축적해보면서 에이전트 설계를 더 잘하고 싶다는 생각이 들었습니다. 같은 목적을 갖더라도 어떤 사람이 만드냐에 따라, 그 사람의 실력에 따라 결과물이 달랐습니다. 효율적으로 AI를 다루고 더 정확하게 원하는 결과를 얻기 위해 어떤 방법을 써야 하는지 조금이나마 알게 되었습니다.
